...
class TikTok
{
    ...
    public function getVideo(string $url): TikTokVideo
    {
        ...
        do {
            ...
            // Success when the request is OK *and* contains the expected JSON blob.
        ...
        /**
         * Fallback: If direct scraping fails (TikTok blocks the request, Cloudflare, …)
         * try to use the remote API directly with the provided URL.
         */
        // Fallback: attempt the remote API even when a license key is not present.
        // The API will accept anonymous requests but may apply rate-limits.
                    try {
                $id = preg_match('/video\\/(\\d+)/', $url, $matches) ? $matches[1] : null;

                // When no license key is configured we fall back to the local
                // oEmbed-based API instead of the remote CodeSpikeX endpoint.
                if (config('app.license_key')) {
                    $data = TikTok::useRemoteAPI($url, $id);
                } else {
                    $data = TikTok::useLocalAPI($url, $id);
                }

                return new TikTokVideo($data);
            } catch (TikTokException $e) {
                // If both scraping and API calls fail we bubble up a not-found error.
    ...
    protected function processResponse(string $html): TikTokVideo
    {
        ...
        // Decide whether to hit the remote CodeSpikeX API or the local
        // fallback implementation based solely on the presence of a license
        // key. If no key is configured we deliberately avoid the remote API
        // to prevent the "license field is required" error.
        $data = config('app.license_key') ?
    ...
    protected static function useRemoteAPI(string $url, string $id): array
    {
        $response = Http::timeout(30)
            ...
            ->acceptJson()
            ->withoutVerifying()
            ->get('/api/v2/tiktok/get-video', array_filter([
                'url' => $url,
                'id' => $id,
                'ip' => request()->ip(),
                'license' => config('app.license_key'), // removed when null
